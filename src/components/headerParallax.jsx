...
const headerParallax = () => {
  ...
  const yRangeText = useTransform(scrollYProgress, [0, 1], [300, 500], {ease: cubicBezier(.14,.8,.4,1)});
  ...
  const yRangeTrans = useTransform(scrollYProgress, [.5, .9], [100, -100], {ease: cubicBezier(.14,.8,.4,1)});
  ...
  return (
    ...
    <motion.div ref={containerRef} className="headerParallax">
      ...
      <motion.span style={{ y: yRangeText, zIndex: 2 }} className="hero-text">Summit Seekers</motion.span>
        <motion.img src={PR_layer_2} className="parallax-layer" style={{ y: yRangeLayer2, zIndex: 3 }} />
        <motion.img src={PR_layer_3} className="parallax-layer" style={{ y: yRangeLayer3, zIndex: 4 }} />
